...
export const mockDatasetInfo = {
  ...
  rows: 12410,
  columnsCount: 30,
  description: "Dữ liệu khí tượng thủy văn đã làm sạch của 34 tỉnh/thành Việt Nam, từ 21/07/2025 đến 20/07/2026, thu thập từ Open-Meteo API và tổng hợp theo ngày.",
  columns: [
    { name: "province", type: "String", nullCount: 0, description: "Tên tỉnh/thành" },
    { name: "region", type: "String", nullCount: 0, description: "Vùng địa lý" },
    { name: "date", type: "Date (YYYY-MM-DD)", nullCount: 0, description: "Ngày quan sát" },
    { name: "temp_mean", type: "Float (°C)", nullCount: 0, description: "Nhiệt độ trung bình" },
    { name: "temp_max", type: "Float (°C)", nullCount: 0, description: "Nhiệt độ cực đại" },
    { name: "temp_min", type: "Float (°C)", nullCount: 0, description: "Nhiệt độ cực tiểu" },
    { name: "app_temp_mean", type: "Float (°C)", nullCount: 0, description: "Nhiệt độ cảm nhận trung bình" },
    { name: "app_temp_max", type: "Float (°C)", nullCount: 0, description: "Nhiệt độ cảm nhận cực đại" },
    { name: "app_temp_min", type: "Float (°C)", nullCount: 0, description: "Nhiệt độ cảm nhận cực tiểu" },
    { name: "precipitation_sum", type: "Float (mm)", nullCount: 0, description: "Tổng lượng mưa" },
    { name: "rain_sum", type: "Float (mm)", nullCount: 0, description: "Lượng mưa" },
    { name: "showers_sum", type: "Float (mm)", nullCount: 0, description: "Lượng mưa rào" },
    { name: "precipitation_hours", type: "Float (h)", nullCount: 0, description: "Số giờ mưa" },
    { name: "humidity_mean", type: "Float (%)", nullCount: 0, description: "Độ ẩm trung bình" },
    { name: "wind_speed_max", type: "Float (km/h)", nullCount: 0, description: "Tốc độ gió lớn nhất" },
    { name: "wind_gusts_max", type: "Float (km/h)", nullCount: 0, description: "Tốc độ gió giật lớn nhất" },
    { name: "shortwave_radiation_sum", type: "Float", nullCount: 0, description: "Bức xạ sóng ngắn" },
    { name: "pressure", type: "Float (hPa)", nullCount: 0, description: "Áp suất khí quyển" },
    { name: "cloud_cover", type: "Float (%)", nullCount: 0, description: "Độ che phủ mây" },
    { name: "dew_point", type: "Float (°C)", nullCount: 0, description: "Điểm sương" },
    { name: "et0", type: "Float (mm)", nullCount: 0, description: "Lượng bốc thoát hơi tham chiếu" },
    { name: "latitude", type: "Float (°)", nullCount: 0, description: "Vĩ độ" },
    { name: "longitude", type: "Float (°)", nullCount: 0, description: "Kinh độ" },
    { name: "sunshine_hours", type: "Float (h)", nullCount: 0, description: "Số giờ nắng" },
    { name: "daylight_hours", type: "Float (h)", nullCount: 0, description: "Số giờ ban ngày" },
    { name: "month", type: "Integer", nullCount: 0, description: "Tháng" },
    { name: "week", type: "Integer", nullCount: 0, description: "Tuần" },
    { name: "weather_code", type: "Integer", nullCount: 0, description: "Mã thời tiết WMO" },
    { name: "wind_direction_10m_dominant", type: "String", nullCount: 0, description: "Hướng gió chính" },
    { name: "season", type: "String", nullCount: 0, description: "Mùa" }
  ],
  sampleRows: [
...
export const predefinedQueries = [
  {
    ...
    question: "So sánh lượng mưa trung bình giữa các vùng miền Việt Nam trong toàn bộ tập dữ liệu.",
    explanation: "Tính toán lượng mưa trung bình theo từng vùng (region) trong khoảng 21/07/2025 đến 20/07/2026, sau đó vẽ biểu đồ cột để so sánh giữa các vùng địa lý.",
    code: `import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns

# 1. Đọc dữ liệu từ file CSV cục bộ
df = pd.read_csv("frontend/public/data/kttv.csv")

# 2. Chuyển đổi cột date sang định dạng datetime
df['date'] = pd.to_datetime(df['date'])

# 3. Lọc toàn bộ khoảng thời gian hiện có của tập dữ liệu
df_q1 = df[(df['date'] >= '2025-07-21') & (df['date'] <= '2026-07-20')]
...
plt.title('Lượng mưa trung bình theo vùng miền', fontsize=14, fontweight='bold', pad=15)
...
`,
    logs: [
      ...
      "[2026-07-22 12:49:06] Loading dataset 'frontend/public/data/kttv.csv' (12,410 rows)...",
      "[2026-07-22 12:49:07] Xử lý dữ liệu theo khoảng thời gian: 2025-07-21 -> 2026-07-20.",
  ...
  },
  {
    ...
    explanation: "Nhóm dữ liệu theo ngày và lọc riêng cho 3 thành phố đại diện 3 miền, sau đó vẽ đồ thị đường biểu diễn sự thay đổi của cột nhiệt độ trung bình (temp_mean).",
    code: `import pandas as pd
import matplotlib.pyplot as plt
...
`,
    logs: [
      ...
      "[2026-07-22 12:51:11] Đọc tệp dữ liệu 'frontend/public/data/kttv.csv' (12,410 rows)...",
    ...
    kpis: [
      ...
      { label: "Nhiệt Độ TB Thấp Nhất", value: "11.2 °C", desc: "Hà Nội (theo tập dữ liệu)", trend: "down" },
  ...
  },
  {
    ...
    explanation: "Trích xuất dữ liệu của tháng 5/2026, tính toán giá trị trung bình của nhiệt độ (temp_mean) và lượng mưa (precipitation_sum) cho từng tỉnh thành, sắp xếp giảm dần theo nhiệt độ và lấy ra top 10 tỉnh đứng đầu.",
    code: `import pandas as pd
...
top_10_hot = prov_stats.sort_values(by='temp_mean', ascending=False).head(10)
...
export const mockDashboardKPIs = [
  { label: "Tập dữ liệu đã nạp", value: "1 tập tin", desc: "kttv.csv", icon: "Database", change: "2.25 MB", trend: "success" },
  { label: "Tổng số bản ghi", value: "12,410 dòng", desc: "30 biến thời tiết và địa lý", icon: "Rows", change: "100% Việt Nam", trend: "success" },
...
export const mockRecentActivities = [
  { id: 1, type: "upload", user: "Phạm Minh", target: "kttv.csv", time: "10 phút trước", status: "success" },
